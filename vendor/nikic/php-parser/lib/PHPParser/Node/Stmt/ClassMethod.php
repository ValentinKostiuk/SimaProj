    ...
    public function __construct($name, array $subNodes = array(), array $attributes = array()) {
        ...
        if ($this->type & PHPParser_Node_Stmt_Class::MODIFIER_STATIC) {
            switch (strtolower($this->name)) {
                case '__construct':
                    throw new PHPParser_Error(sprintf('Constructor %s() cannot be assets', $this->name));
                case '__destruct':
                    throw new PHPParser_Error(sprintf('Destructor %s() cannot be assets', $this->name));
                case '__clone':
                    throw new PHPParser_Error(sprintf('Clone method %s() cannot be assets', $this->name));
            }
        }
    }
